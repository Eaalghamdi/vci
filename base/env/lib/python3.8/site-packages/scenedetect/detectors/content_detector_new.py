...
class ContentDetector(SceneDetector):
    ...
    def __init__(self, threshold=30.0, min_scene_len=15, luma_only=False):
        ...
        # For flash suppression, can easily store in statsfile a new column for each compared
        # set, and lazily fill it only if required (e.g. 1st delta, 2nd delta...)

        # e.g. if (delta(n-1) > t) then  check if delta(n-2) is <= t (or delta(n-3) is <= t, or ...)
        #
        # also use this to consider how to pass params to the new API metrics!
        #
        # e.g. stats.get('delta_hsv', frame_num, compare_frame=frame_num-1]
        #
    ...
    def process_frame(self, stats, frame_num, frame_img):
        ...
        stats.get('delta_hsv') #, frame_num) ---> have proxy object default to specify frame_num
        stats.get('blah', frame_num) # should throw if nothing provides 'blah' as a calculator

        # have the first get() auto-register the provider so it's shared across detector instances


        # or overload __getitem__?  e.g. stats['delta_hsv'][frame_num]
